(function (wHandle, wjQuery) {
        ...
        wjQuery.get(
            ...
            function (a) {
                ...
                for (var numplayer in numPlayers)
                    wjQuery('#region option[value="' + numplayer + '"]').text(
                        playerStat[numplayer] +
                            " (" +
    ...
    function Ha() {
        wjQuery("#region").val()
            ? (wHandle.localStorage.location = wjQuery("#region").val())
            : wHandle.localStorage.location &&
              wjQuery("#region").val(wHandle.localStorage.location);
        wjQuery("#region").val()
            ? wjQuery("#locationKnown").append(wjQuery("#region"))
            : wjQuery("#locationUnknown").append(wjQuery("#region"));
    }
    ...
    function calcViewZoom() {
        if (0 != playerCells.length) {
            ...
            newViewZoom =
                Math.pow(Math.min(64 / newViewZoom, 1), 0.4) * viewRange();
    ...
    function drawGameScene() {
        ...
        if (0 != userScore) {
            ...
            scoreText.setValue("Масса: " + ~~(userScore / 100));
            c = scoreText.render();
            a = c.width;
            ctx.globalAlpha = 0.2;
    ...
    Cell.prototype = {
        ...
        shouldRender: function () {
            if (0 == this.id) {
                ...
                return !(
                    this.x + this.size + 40 <
                        nodeX - canvasWidth / 2 / viewZoom ||
                    this.y + this.size + 40 <
                        nodeY - canvasHeight / 2 / viewZoom ||
                    this.x - this.size - 40 >
                        nodeX + canvasWidth / 2 / viewZoom ||
                    this.y - this.size - 40 >
                        nodeY + canvasHeight / 2 / viewZoom
            ...
            if (this.shouldRender()) {
                ...
                if (!this.isAgitated && showSkin && ":teams" != gameMode) {
                    if (-1 != knownNameDict.indexOf(skinName)) {
                        ...
                        if (
                            0 != skins[skinName].width &&
                            skins[skinName].complete
                        ) {
                            c = skins[skinName];
                        } else {
                            c = null;
                ...
                if (0 != this.id) {
                    ...
                    if (
                        ...
                        this.nameCache &&
                        (null == e ||
                            -1 == knownNameDict_noDisp.indexOf(skinName))
                    ) {
            ...
            Node.prototype = {
                ...
                exists: function (selector) {
                    ...
                    if (0 != this.nodes.length) {
                        var self = this;
                        return this.findOverlappingNodes(
                            selector,
                            function (dir) {
