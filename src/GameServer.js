...
<input type="text" id="chat_textbox" maxlength="200" placeholder="Нажимте Enter для чата!"/>
...
`
    ...
    function connectionEstablished(ws) {
        if (this.clients.length > this.config.serverMaxConnections) {
            ws.close();
            console.log(
                "[Game] Client tried to connect, but server player limit has been reached!"
            );
            return;
        }
...
GameServer.prototype.getRandomPosition = function () {
    return {
        x:
            Math.floor(
                Math.random() *
                    (this.config.borderRight - this.config.borderLeft)
            ) + this.config.borderLeft,
        y:
            Math.floor(
                Math.random() *
                    (this.config.borderBottom - this.config.borderTop)
...
GameServer.prototype.sendMessage = function (msg) {
    for (var i = 0; i < this.clients.length; i++) {
        ...
        this.clients[i].playerTracker.socket.sendPacket(
            new Packet.Message(msg)
        );
    }
};
...
GameServer.prototype.spawnFood = function () {
    var f = new Entity.Food(
        ...
        Math.floor(Math.random() * this.config.foodMaxMass) +
...
GameServer.prototype.spawnPlayer = function (client) {
    ...
    var cell = new Entity.PlayerCell(
        this.getNextNodeId(),
        client,
        pos,
        startMass
...
GameServer.prototype.updateMoveEngine = function () {
    ...
    for (var i = 0; i < len; i++) {
        ...
        if (check.getMoveTicks() > 0) {
            ...
            if (
                check.getType() == 3 &&
                this.nodesVirus.length < this.config.virusMaxAmount
            ) {
                // Check for viruses
                var v = this.getNearestVirus(check);
                if (v) {
                    // Feeds the virus if it exists
                    v.feed(check, this);
                }
            }
        } else {
...
GameServer.prototype.splitCells = function (client) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        var startPos = {
            x:
                cell.position.x +
                (size + this.config.ejectMass) * Math.sin(angle),
            y:
                cell.position.y +
                (size + this.config.ejectMass) * Math.cos(angle),
        };
        // Calculate mass of splitting cell
        var newMass = cell.mass / 2;
        cell.mass = newMass;
        // Create cell
        split = new Entity.PlayerCell(
            this.getNextNodeId(),
            client,
            startPos,
            newMass
    ...
    for (var i = 0; i < client.cells.length; i++) {
        ...
        if (!this.config.ejectVirus) {
            ejected = new Entity.EjectedMass(
                ...
                null,
                startPos,
                this.config.ejectMassGain
            );
        } else {
            ejected = new Entity.Virus(
                this.getNextNodeId(),
                null,
                startPos,
                this.config.ejectMassGain
    ...
    for (var i = 0; i < len; i++) {
        ...
        switch (check.getType()) {
            ...
            case 2: // Virus
                ...
                break;
            case 0: // Players
                multiplier = check.owner == cell.owner ? 1.0 : multiplier;
...
GameServer.prototype.updateCells = function () {
    var massDecay =
        1 - (this.config.playerMassDecayRate / 1000) * this.gameMode.decayMod;
    for (var i = 0; i < this.nodesPlayer.length; i++) {
        var cell = this.nodesPlayer[i];
